...
@Component({
  selector: 'app-abogados-form-component',
  imports: [
    FormsModule,
    ReactiveFormsModule,
  ...
  templateUrl: './abogados-form-component.html',
  styleUrl: './abogados-form-component.css',
})
export class AbogadosFormComponent implements OnInit {
  @Input()
  ...
  formGroupCompleto: FormGroup = new FormGroup({
    ...
    transcurso_De_La_Llamada: new FormGroup({
      ...
      cualificacion_profesional_del_abogado: new FormControl(0),
      proactividad_con_cliente_anticipacion_identificacion_problema: new FormControl(0),
    }),
  ...
  private calcularResultadoSeccion(sectionKey: string): void {
    ...
    if (!formGroup) return;
    const pesosActuales = WEIGHTSABOGADOS;
