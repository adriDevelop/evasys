...
import { WEIGHTSINBOUD } from '../../../core/environments/data-results';

@Component({
  selector: 'app-inboud-form-component',
  imports: [
    FormsModule,
    ReactiveFormsModule,
  ...
  templateUrl: './inboud-form-component.html',
  styleUrl: './inboud-form-component.css',
})
export class InboudFormComponent {
  @Input()
  ...
  formGroupCompleto: FormGroup = new FormGroup({
    ...
    comunicacion_Con_El_Cliente: new FormGroup({
      ...
      seguridad_respuesta: new FormControl(0),
    }),
    ...
    venta_Cruzada: new FormGroup({
      socios_oferta_family_duo_mgm: new FormControl(0),
      no_socios: new FormControl(0),
      oferta_primer_lugar_cobro_tarjeta_pago_anual: new FormControl(0),
      arguentario_tarificaciones_bajas_altas: new FormControl(0),
    }),
    gestion_De_La_Actividad: new FormGroup({
      crea_contacto_crm_anotaciones_contacto: new FormControl(0),
  ...
  formGroupResultado: FormGroup = new FormGroup({
    ...
    venta_cruzada_resultado: new FormControl(0),
  ...
  private calcularResultadoSeccion(sectionKey: string): void {
    ...
    if (!formGroup) return;
    const pesosActuales = WEIGHTSINBOUD;
